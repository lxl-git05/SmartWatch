...
void  LCD_WriteCommand(uint8_t lcd_command)
{
   ...
   HAL_SPI_Transmit(&hspi4, &lcd_command, 1, 1000); // 启动SPI传输
}
...
void  LCD_WriteData_8bit(uint8_t lcd_data)
{
   ...
   HAL_SPI_Transmit(&hspi4, &lcd_data, 1, 1000) ; // 启动SPI传输
}
...
void  LCD_WriteData_16bit(uint16_t lcd_data)
{
   ...
   lcd_data_buff[1] = lcd_data;
	...
	HAL_SPI_Transmit(&hspi4, lcd_data_buff, 2, 1000) ;   // 启动SPI传输
}
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   hspi4.Init.DataSize 	= SPI_DATASIZE_16BIT;   //	16位数据宽度
   HAL_SPI_Init(&hspi4);		
	
	HAL_SPI_Transmit(&hspi4, (uint8_t *)DataBuff, DataSize, 1000) ; // 启动SPI传输
	
// 改回8位数据宽度，因为指令和部分数据都是按照8位传输的
	hspi4.Init.DataSize 	= SPI_DATASIZE_8BIT;    //	8位数据宽度
   HAL_SPI_Init(&hspi4);	
}
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   LCD_SPI_Transmit(&hspi4, LCD.BackColor, LCD.Width * LCD.Height) ;   // 启动传输
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   LCD_SPI_Transmit(&hspi4, LCD.BackColor, width*height) ;  // 启动传输
...
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0x000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayString( 10, 10, "111KE") ，在起始坐标为(10,10)的地方显示字符串"111KE"
*
*****************************************************************************************************************************************/
...
*					2.	可设置要显示的颜色，例如使用 LCD_SetColor(0xff0000FF) 设置为蓝色
*					3. 可设置对应的背景色，例如使用 LCD_SetBackColor(0xff000000) 设置为黑色的背景色
*					4. 使用示例 LCD_DisplayChinese( 10, 10, "111科技STM32") ，在坐标(10,10)显示字符串"111科技STM32"
*
**********************************************************************************************************************************fanke*******/
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   LCD_SPI_Transmit(&hspi4, LCD.Color, width*height) ;
...
// 修改为16位数据宽度，写入数据更加效率，不需要拆分	
   ...
   HAL_SPI_Init(&hspi4);		
	...
	LCD_SPI_TransmitBuffer(&hspi4, DataBuff,width * height) ;
 ...
 void LCD_SPI_CloseTransfer(SPI_HandleTypeDef *hspi)
{
  ...
  /* SPI Mode 111ult error interrupt occurred -------------------------------*/
